...
with open(budget_file) as csvfile:
    ...
    for row in budgetreader:
        ...
        # take the current month's p&l and subtract the previous month's p&l for change in pl. 
        ...
        # find the greatest increase in p&l change and the month
        if pl_change > greatest_increase[1]:
            greatest_increase[1] = pl_change
            greatest_increase[0] = row[0]

        # find the greatest decrease in p&l change and the month
        if pl_change < greatest_decrease[1]:
            greatest_decrease[1] = pl_change
            greatest_decrease[0] = row[0]


    # average is calculated by adding each p&l change and then dividing by the count
    # take the p&l change list gathered above for the formula
